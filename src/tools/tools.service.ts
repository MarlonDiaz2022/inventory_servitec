...
export class ToolsService {
  ...
  async deletetools(code: string) {
    ...
    if (!existsUser) {
      throw new ConflictException(`herramienta con codigo ${code} no existe`);
    }
    ...
    if (element != null) {
      if (isNaN(Number(element.amount))) {
        throw new NotFoundException(
          `herramineta con codigo ${id} has an invalid amount value`,
        );
      }
      if (element.amount <= 0) {
  ...
  async increaseamount(id: String, increase = 1) {
    ...
    if (element != null) {
      if (isNaN(Number(element.amount))) {
        throw new NotFoundException(
          `Tool with id ${id} has an invalid amount value`,
        );
      }
  ...
  async ismaintenence(code: string) {
    ...
    if ((element.maintenance == false)) {
      element.maintenance = true;
    } else {
      element.maintenance = false;
    }
    await element.save();
  }
}
